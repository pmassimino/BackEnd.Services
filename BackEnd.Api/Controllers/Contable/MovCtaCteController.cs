...
namespace BackEnd.Api.Controllers.Contable
{
    ...
    [ApiController]
    public class MovCtaCteController : ApiController<MovCtaCte, Guid> {

        IMovCtaCteService _service;
        ...
        public MovCtaCteController(IMovCtaCteService service,ISessionService sessionService,IEmpresaService empresaService,ISujetoService sujetoService, IWebHostEnvironment hostingEnvironment, IAuthService authService) : base(service, authService)
        {
        ...
        [HttpGet("ResumenSaldo")]
        public IActionResult ResumenSaldo(DateTime? fecha,string idCuenta="",string idCuentaMayor = "" )
        {
            ...
            return Ok(this._service.ResumenSaldo(pfecha,idCuenta,idCuentaMayor));
        }
        [HttpGet("Resumen/{id}")]
        public IActionResult Resumen(string id , string idCuentaMayor, DateTime? fecha,DateTime? fechaHasta)
        {
            DateTime pfecha = DateTime.Now.AddDays(-30);
            DateTime pfechaHasta = DateTime.Now;
            if (fecha.HasValue)
            {
                pfecha = (DateTime)fecha;
            }
            ...
            return Ok(this._service.Resumen(id,idCuentaMayor,pfecha,pfechaHasta));
        }
        [HttpGet("ResumenPdf/{id}")]
        public async Task<IActionResult> ResumenPdf(string id, string idCuentaMayor, DateTime? fecha = null, DateTime? fechaHasta = null)
        {
            DateTime pfecha = DateTime.Now.AddDays(-30);
            DateTime pfechaHasta = DateTime.Now;
            if (fecha.HasValue)
            ...
            var tmpMovCtaCte = this._service.Resumen(id, idCuentaMayor, pfecha, pfechaHasta);
            var tmpCuenta = this.sujetoService.GetOne(id);
            ...
            cell.BorderWidthRight = 0;
            Table.AddCell(cell);
            cell = new PdfPCell(new Phrase("U$S", font));
            cell.BorderWidthLeft = 0;
            cell.BorderWidthRight = 0;
            Table.AddCell(cell);
            ...
            foreach (var item in tmpMovCtaCte)
            {
                BaseColor colorBack;
                ...
                cell = new PdfPCell(new Phrase(item.MovCtaCte.FechaComp.ToShortDateString(), font));
                cell.BackgroundColor = colorBack;
                cell.Border = borderVenc;
                cell.BorderColor = BaseColor.WHITE;
                Table.AddCell(cell);
                cell = new PdfPCell(new Phrase(item.MovCtaCte.Concepto, fontDes));
                cell.BackgroundColor = colorBack;
                cell.Border = borderVenc;
                cell.BorderColor = BaseColor.WHITE;
                Table.AddCell(cell);               
                cell = new PdfPCell(new Phrase(item.MovCtaCte.Numero.ToString(), font));
                cell.BackgroundColor = colorBack;
                cell.Border = borderVenc;
                cell.BorderColor = BaseColor.WHITE;
                Table.AddCell(cell);
                string impd = "";
                //if (item.impD > 0) impd = ((item.Debe - item.Haber) / (double)item.Cotizacion).ToString["N"];
                cell = new PdfPCell(new Phrase(impd, font));
                cell.BackgroundColor = colorBack;
                cell.Border = borderVenc;
                cell.BorderColor = BaseColor.WHITE;
                Table.AddCell(cell);
                cell = new PdfPCell(new Phrase(item.MovCtaCte.FechaVenc.ToShortDateString(), fontDes));
                cell.BackgroundColor = colorBack;
                cell.Border = borderVenc;
                cell.BorderColor = BaseColor.WHITE;
                Table.AddCell(cell);
            ...
            decimal saldo = 0;
            saldoVencido = this._service.SaldoVencido(id,idCuentaMayor,pfechaHasta);
            saldo = this._service.Saldo(id, idCuentaMayor, pfechaHasta);
            saldoaVencer = saldo - saldoVencido;
            // Sumas debe - haber saldo
            var totalDebe = tmpMovCtaCte.Sum(s => s.Debe);
            var totalHaber = tmpMovCtaCte.Sum(s => s.Haber);
            ...
            cell = new PdfPCell(new Phrase(tipoSaldo, fontD));
            cell.Border = 0;
            cell.Colspan = 2;
            cell.HorizontalAlignment = Element.ALIGN_RIGHT;
            Table.AddCell(cell);
            ...
            // Nueva Linea
            Table.AddCell(cell);
            cell = new PdfPCell(new Phrase("A VENCER", fontD));
            cell.Border = 0;
            Table.AddCell(cell);
            ...
            try
            {
                ...
                textEvents.Titulo1 = "RESUMEN DE CTA. CTE.";
                textEvents.Cuenta = tmpCuenta;
                textEvents.FechaDesde = pfecha;
                textEvents.FechaHasta = pfechaHasta;
    ...
    public class ITextEvents : PdfPageEventHelper
    {
        ...
        public override void OnEndPage(PdfWriter writer, Document document)
        {
            ...
            //cell = new PdfPCell(new Phrase("CPostal" + "-" + "this.Cuenta.IdLocalidad.Trim()" + "-" + "this.Cuenta.IdLocalidad.Trim()", baseFontLite));
            cell.Colspan = 2;
            cell.Border = 0;
            Table.AddCell(cell);
